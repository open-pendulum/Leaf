...
static GLenum ShaderTypeFromString(const std::string &type) {
    ...
    LEAF_CORE_ASSERT(false, "Unknown shader type!");
    return 0;
}
}  // namespace
...
std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(
    ...
    while (pos != std::string::npos) {
        ...
        LEAF_CORE_ASSERT(eol != std::string::npos, "Syntax error");
        size_t begin = pos + typeTokenLength + 1;
        std::string type = source.substr(begin, eol - begin);
        LEAF_CORE_ASSERT(ShaderTypeFromString(type),
...
void OpenGLShader::Compile(
    ...
    for (auto &kv : shaderSources) {
        ...
        if (isCompiled == GL_FALSE) {
            ...
            LEAF_CORE_ERROR("{0}", infoLog.data());
            LEAF_CORE_ASSERT(false, "Shader compilation failure!");
            break;
        }
